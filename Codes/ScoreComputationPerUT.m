...
for tidx = 1:ft-it+1
    ...
    for m=1:n_models
        ...
        for i = 1:n_agents
            ct = model_agent_combination{m}.agents{i}.choice_data(:,10,t); 
            % initialize:
            likelihood_model_agent = cell(n_models,1);
            likelihood_models = nan(R,n_models); % marginalizing over agents within model
            for hm=1:n_models % hypothesized models
                ...
                for hi = 1:n_h_agents % hypothesized agents
                    DeltaU = model_agent_combination{hm}.agents{hi}.all_options(t,9);
                    alpha = model_agent_combination{hm}.agents{hi}.pars(1);
